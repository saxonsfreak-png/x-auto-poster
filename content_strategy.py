...
def validate_post_text(text: str, content_config: dict[str, Any]) -> str | None:
    ...
    min_chars = content_config["min_chars"]
    max_chars = content_config["max_chars"]
    # Xの投稿文字数上限(通常アカウントは280文字。Premium等で上限が違う場合は
    # config.yaml の content.max_platform_chars で上書き可能)。
    # これは「読みやすさのためのスタイル上限」(max_chars)とは別に、
    # 超えると投稿自体が成立しない/弾かれる絶対的な上限として扱う。
    max_platform_chars = content_config.get("max_platform_chars", 280)
    length = len(text)

    if length > max_platform_chars:
        return f"X側の投稿文字数上限({max_platform_chars}文字)を超えています({length}文字)"
