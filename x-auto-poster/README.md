...
X(旧Twitter)向けに、Geminiで生成した「副業初心者向け」のテキスト+画像投稿を、
1日3回(既定: 07:25 / 11:55 / 19:55 JST、`config.yaml` で変更可)、
人の手を介さずGitHub Actions経由で自動投稿するパイプラインです。
...
結果として、「完全無人・1日3回・画像付き・保守しやすい構成」というご要望はそのまま実現しつつ、規約違反にならない形にしています。
...
src/
  config.py               # 設定読み込み
  gemini_client.py         # Gemini API(テキスト・画像生成)
...
**処理の流れ**: GitHub Actionsが1日1回(06:00 JST)起動 → その日の3時刻それぞれについて、Buffer上の自己分析データを踏まえてGeminiがテキストと画像を生成 → 画像をCloudinaryにアップロード → Buffer APIで「その時刻に投稿」を予約 → 実際の投稿(Xへの公開)は指定時刻にBufferが実行、という流れです。

## 必要なアカウント(すべて無料枠で開始可能)

| サービス | 用途 | 無料枠の目安 |
|---|---|---|
| [Google AI Studio](https://aistudio.google.com/apikey) | Gemini APIキー取得(テキスト・画像生成) | 無料枠あり(モデルや時期により変動。下記「注意点」参照) |
| [Buffer](https://buffer.com) | Xへの投稿予約・実行、実績取得 | Freeプラン: 3チャンネル、チャンネルあたり予約10件まで、API 3,000回/30日 |
| [Cloudinary](https://cloudinary.com/users/register/free) | Gemini生成画像をBufferに渡す公開URL化 | 無料枠 25クレジット/月(画像数百KB程度なら十分すぎる余裕) |
...
[Google AI Studio](https://aistudio.google.com/apikey) でAPIキーを発行します。

### 2. Buffer

1. [Buffer](https://buffer.com) にサインアップし、投稿したいXアカウントを接続します。
2. [Settings → API](https://publish.buffer.com/settings/api) でPersonal Access Tokenを発行します。
3. ローカルに `.env` を作成し(`cp .env.example .env`)、`BUFFER_API_KEY` だけ先に設定します。
4. 以下を実行して組織ID・チャンネルIDを確認します。
...
5. 表示された `BUFFER_ORGANIZATION_ID` と(Xチャンネルの)`BUFFER_CHANNEL_ID` を `.env` に追記します。
...
実際にBufferへ予約を作成せず、Gemini生成〜Cloudinaryアップロードまでを確認できます。
...
### 5. GitHub Actionsへの登録

1. このプロジェクトをGitHubリポジトリにpushします(`.env` は `.gitignore` 済みなのでpushされません)。
2. リポジトリの **Settings → Secrets and variables → Actions** で、`.env.example` に列挙されているのと同じ名前で7つのSecretsを登録します。
...
- `gemini.text_model` / `image_model`: 使用するモデル名。

## 運用上の注意点

- **Geminiのモデル名は数か月単位で変わります。** 実行前に一度 [ai.google.dev/gemini-api/docs/models](https://ai.google.dev/gemini-api/docs/models) で現行モデルと無料枠の条件を確認し、必要なら `config.yaml` を更新してください。
